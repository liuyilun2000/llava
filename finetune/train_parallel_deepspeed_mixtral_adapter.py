...
#deepspeed llava/train_parallel_deepspeed_mixtral_adapter.py --num_stages=8 2>&1 | tee llava/train_parallel_deepspeed_mixtral_adapter_lora.log
...
#        shared_adapter_args={'r': 256, 'lora_alpha': 32, 'lora_dropout': 0.1},
        ...
        shared_sparse_adapter_args={'r': 32, 'lora_alpha': 32, 'lora_dropout': 0.1},
    ...
    for shard_id in tqdm(range(NUM_DATASET_SHARDS)):
        if shard_id < args.skip_shard:
            ...
            continue
        print_rank_0("Loading dataset from "+WORK_DIR+'LLaVA-Pretrain_processed_dataset/shard_'+str(shard_id), rank)
        processed_dataset = load_from_disk(WORK_DIR+'LLaVA-Pretrain_processed_dataset/shard_'+str(shard_id))
