...
model_name = 'LLaVA-PEFT_adapter_32_64_4_top1'
pretrained_model_dir = '/home/vault/b207dd/b207dd11/llava-mixtral/llava-mixtral-pretrained-2/'
pipeline_model_dir = f'/home/atuin/b207dd/b207dd11/{model_name}_checkpoint/'

checkpoint_name = 'save_step1119'
...
#        shared_adapter_args=shared_adapter_args,
    ...
    output_router_logits=False#True
)
...
save_file(adapter_static_dict, save_dir+"/adapter.safetensors")
...
for name, param in model.named_parameters():
    ...
    found_key_adapter = find_key_substring_in_name(name, adapter_static_dict)
    if found_key_multi_modal:
        ...
        mm_count += 1
    elif found_key_adapter:
        print(f"Name: {name}, Found Key in adapter_static_dict: {found_key_adapter}")
        model.state_dict()[name].copy_(adapter_static_dict[found_key_adapter])
        adapter_count += 1
...
adapter_static_dict = {}
with safe_open(save_dir+"/adapter.safetensors", framework="pt", device='cpu') as f:
    for k in f.keys():
