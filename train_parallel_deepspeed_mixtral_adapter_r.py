...
from deepspeed_pipeline_model_r import *
...
def set_args():
    ...
    #
    parser.add_argument("--shared_routing_adapter_type", type=str, default="", help="")
    parser.add_argument("--shared_routing_adapter_num_experts", type=int, default=0, help="")
    parser.add_argument("--shared_routing_adapter_num_experts_per_tok", type=int, default=0, help="")
    #
    ...
    #parser.add_argument("--wall_clock_breakdown", default=False, type=bool, help="")
...
def init_trainable_parameters(model, verbose=False):
    for name, module in model.named_modules():
        if hasattr(module, 'lora_A') and hasattr(module, 'lora_B'):
            if verbose:
                print(name, module)
                print(f"Initializing LoRA matrices for {name}")
        ...
        if hasattr(module, 'adapter_w1') and hasattr(module, 'adapter_w2'):
            if verbose:
                print(name, module)
                print(f"Initializing adapter matrices for {name}")
...
def main():
    ...
    if args.shared_routing_adapter_type == 'LoRA':
        shared_routing_adapter_args = {'r': args.lora_r, 'lora_alpha': args.lora_alpha, 'lora_dropout': 0.1}
    elif args.shared_routing_adapter_type == "Parallel_Adapter":
        shared_routing_adapter_args = {'hidden_dim': args.hidden_dim, 'hidden_act': 'silu', 'dropout': 0.1}
...
#        shared_adapter_args=shared_adapter_args,
        ...
        shared_routing_adapter_num_experts=args.shared_routing_adapter_num_experts,
        shared_routing_adapter_num_experts_per_tok=args.shared_routing_adapter_num_experts_per_tok,
        shared_routing_adapter_type=args.shared_routing_adapter_type,
        shared_routing_adapter_args=shared_routing_adapter_args,
    ...
    shard_record_path = f"{args.checkpoint_dir}/shard_record.json"
    shard_record = []

    # Read existing records if the file already exists
    if os.path.exists(shard_record_path):
        with open(shard_record_path, "r") as f:
            try:
                shard_record = json.load(f)
            except json.JSONDecodeError:
                shard_record = []

    last_processed_shard_id = max([record["shard_id"] for record in shard_record], default=-1)

    
    for shard_id in tqdm(range(len(dataset_shards))):
        if shard_id < args.skip_shard or shard_id <= last_processed_shard_id:
            #print_rank_0(f"Shard {shard_id} skipped", rank)
            continue
        ...
        try:
            ...
            if (shard_id + 1) % args.save_model_shard == 0:
                ...
                shard_record.append({"shard_id": shard_id})
                with open(shard_record_path, "w") as f:
                    json.dump(shard_record, f, indent=4)
